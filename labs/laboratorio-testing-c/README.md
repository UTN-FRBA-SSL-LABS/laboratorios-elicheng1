...
> R: arranca limpio en 0, pone el contador 0
...
> R:linea 24 (total += c->items[i].precio;), solo suma precio, falta multiplicar

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:total += c->items[i].precio * c->items[i].cantidad; 
...
> R:#####:   29:int carrito_descuento(int total, int porcentaje) {
    #####:   30:    return total - (total * porcentaje / 100);
